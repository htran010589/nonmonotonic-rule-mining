...
RUMIS stands for Nonmonotonic Rule Mining System which is the main tool developed from the current work. In this chapter, our system describes practical implementation for the current work in details with the following organization. First, an overview of the system is discussed. Second, the implementation of main components in the system are presented. Finally, we talk about installation and usage this system.

\section{System Overview}
\label{sec:overview}

There are six components in the system as indicated in Figure~\ref{system_overview}. RUMIS finds the nonmonotonic rules in the following steps:

\begin{itemize}
\item In pipeline 1, a KG $\cG$ as an input is passed to the system. First, $\cG$ is stored and indexed to the database by Component 1.
\item In pipeline 2, Horn rules of the form \textit{h(X, Z) $\leftarrow$ p(X, Y), q(Y, Z)} are mined by Component 2 using Database Indexing.
\item In pipeline 3, normal and abnormal sets are found by Component 3.
\item In pipeline 4, exception witness sets are found based on normal and abnormal sets in the previous step.
\item In pipelines 5 and 6, exception candidates are ranked in Component 6 using a measure plugged by Component 5. RUMIS allocates a separate component for measure plugin to flexibly extend ranking criteria in the future.
\item In pipeline 7, after being ranked, the best revision for each Horn rule is added to the output of RUMIS.
\end{itemize}
...
In this section, the main components mentioned in Section~\ref{sec:overview} are described in details. To simplify the problem of \textit{language bias}, the implementation of RUMIS only take the following positive rule form into consideration:
...
Since the KG $\cG$ can be huge, it takes a long time to find instance and exception witness sets of a positive rule. To overcome this, data indexing is taken into consideration to speed up processing data. [may talk more about intuition of indexing in traditional search engine]. Figure~\ref{data_indexing} illustrates data indexed from part of KG in Figure~\ref{fig1.1}. In Figure~\ref{data_indexing}, based on the index, if predicate \textit{isMarriedTo} is given, we can find set of subject-object pairs corresponding to this relation, i.e., \textit{<Brad, Ann>} and \textit{<John, Kate>}. [can extend more with examples in the figure].
...
[ToDo: Describe in genaral PM and OPM ranking]

Figure~\ref{pm_ranking} shows an intuition for PM and OPM ranking.
...
\textbf{PM Ranking.} [ToDo: Describe PM ranking]
...
\textbf{OPM Ranking.} [ToDo: Describe OPM ranking]
