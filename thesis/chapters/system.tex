...
To be specific, Figure~\ref{data_indexing} illustrates data indexed from part of KG in Figure~\ref{fig1.1}. In the figure, based on the index, if predicate \textit{isMarriedTo} is given, we can find set of subject-object pairs corresponding to this relation, i.e., \textit{$<$Brad, Ann$>$} and \textit{$<$John, Kate$>$}. [can extend more with examples in the figure].

Data Indexing provides the following functions that are exploited in the rest components:

\begin{itemize}
\item \textit{getPredicateSubjectSet($\cG$, object):} This function is to return a set of predicate-subject pairs corresponding to a given \textit{object} in KG $\cG$.
\item \textit{getPredicateSet($\cG$, subject, object):} This function is to return a set of predicates corresponding to given \textit{subject, object} entiries in KG $\cG$.
\item \textit{getSubjectSet($\cG$, predicate, object):} This function is to return a set of subjects corresponding to given \textit{predicate, object} in KG $\cG$.
\item \textit{getSubjectObjectSet($\cG$, predicate):} This function is to return a set of subject-object pairs corresponding to given \textit{predicate} in KG $\cG$.
\end{itemize}

[Explain how to add and remove indexes in data indexing component.]

\subsection{Positive Rule Mining}

Positive Rule Mining is to find Horn rules with the largest \textit{absolute support}. The way how this works is illustrated in Algorithm~\ref{algo1} with the following steps:

\begin{itemize}
\item First, in line 1, \textit{absSupp} is created with the aim to store absolute support of positive rules. More specifically, $absSupp[hpq]$ is the absolute support of rule \textit{r2}.
\item Second, in line 2 and 3, for each triple \textit{$<$Y q Z$>$} from KG $\cG$, a set of pairs \textit{X, p} are found such that \textit{X p Y} is in $\cG$. Command in line 3 is executed using \textit{getPredicateSubjectSet} function in Data Indexing component.
\item Third, from line 5 to 6, with \textit{X, Z} found in the previous step, RUMIS searchs for every relation $h$ such that \textit{$<$X h Z$>$} is in $\cG$. At this point, it is guaranteed that \textit{h(X, Z), p(X, Y), q(Y, Z)}. After that, in line 7, absolute support of the rule \textit{r2} is increased by $1$.
\item Finally, all rules are sorted in decreasing order of absolute support.
\end{itemize}
...
This component is to find normal and abnormal sets of rule \textit{r2} with the following steps:

\begin{itemize}
\item First, from line 1 to 2, normal and abnormal sets are created.
\item Second, from line 3 to 5, for each pair \textit{Y, Z} such that \textit{$<$Y q Z$>$} is in $\cG$, a set of $X$ such that \textit{$<$X p Y$>$} is in $\cG$ are found based on Data Indexing component. At this point, it is guaranteed that \textit{X, Y, Z}  fulfills the body of \textit{r2}.
\item Finally, for every $X$ mentioned in the previous step, \textit{$<$X h Z$>$} is verified to be in the the KG or not. If it is in $\cG$, \textit{$<$X Z$>$} is added to normal set, otherwise it is added to abnormal set.
\end{itemize}
...
This component is to find EWS for each positive rule with the form \textit{r2}. Algorithm~\ref{algo3} presents the way how this component is processed with binary predicate exceptions [specify more]. Unary exceptions can be mined in the similar way with binary ones. There are several steps in EWS mining as follows.

\begin{itemize}
\item First, normal and abnormal instance sets are found from Algorithm~\ref{algo2} in line 1 and 2. In addition, from line 3 to 4, $EWS+$ and $EWS-$ (i.e., set of relations between \textit{$<$X Z$>$} in the abnormal and normal sets, resp.) are created.
\item Second, From line 5 to 8, for every pair \textit{$<$X Z$>$} in the abnormal set, all relations between them are added to the $EWS+$. Similar stage for $EWS-$ from line 9 to 12.
\item Finally, exception witness set $EWS$ is calculated as the difference between $EWS+$ and $EWS-$.
\end{itemize}

\subsection{Measure Plugin}

To be easy to plugin and test another quality measure, the measure plugin is designed as seperate component. The main measure used in the system is conviction. It is guaranteed that conviction has high predictive quality, thus, the final rule revisions are expected to be highly accurate.

\subsection{Exception Ranking}

Figure~\ref{pm_ranking} shows the communication between different components in partial materialization ranking. The communications are expressed by arrows in the figure.

\begin{itemize}
\item Arrow 1 shows that from a KG and a set of Horn rules, EWS Mining is executed to find exception candidates for each Horn rule.
\item Arrow 2 shows that some rules with exceptions are used to infer new facts from the original KG.
\item Arrow 4[???] shows that original KG with new predicted facts are used to rank exceptions for each Horn rule. The combination of step 2 and 4 describe the interaction of different rules, that is also the intuition of partial materialization.
\item Arrow 3[???] is a process that exceptions are ranked by Component 5, the best exception is chosen for the revision of a rule.
\end{itemize}
...
\textbf{PM Ranking.} PM ranking is executed in the following steps:

\begin{itemize}
\item From line 1 to 2, we clone an original graph $\cG$ to $\cG'$ and create empty nonmonotonic rule set.
\item From line 3 to 5, for every Horn rule $r$, all facts inferred from $r$ and not from all exception candidates of $r$[should be defined in the background], its revisions are applied to $\cG$ to predict new facts. These facts are then added to $\cG'$ using data indexing component.
\item For each Horn rule $r$, indexes of facts predicted by apply this rule to $\cG$ are removed from $\cG'$ in line 7. This step is to make sure that predicted facts of all other rules ($r$ excluded) are exploited to determine quality of $r$ (line 8). Thanks to this, the interaction between nonmonotonic rules are taken into consideration for the ranking. After that, the best revision of $r$ is added to the final result in line 9. Line 10 shows a step that undoes what is done in line 7, i.e., predicted facts of rule $r$ based on $\cG$ is added to $\cG'$ again. This guarantees in the next iteration can be processed similarly with a new rule.
\end{itemize}
...
\BlankLine
\ForEach{Rule $r$ in $R_H$} {
	Predict new facts by applying all revisions of $r$ to $\cG$ then remove these facts' indexes from $\cG'$\\
...
\textbf{OPM Ranking.} The OPM is processed as follows:

\begin{itemize}
\item From line 1 to 2, the process is executed the same in PM ranking. In line 3, all Horn rules are sorted in decreasing order of conviction measure.
\item From line 4 to 8, for each Horn rule $r$, all facts inferred from previous rules with all exceptions based on $\cG$ are added to $\cG'$ and used to assess quality of $r$. Similar to PM ranking, the best revision is chosen for the final result.
\end{itemize}
