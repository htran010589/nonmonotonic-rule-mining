...
RUMIS is developed and currently tested in Linux~\footnote{\url{https://github.com/htran010589/nonmonotonic-rule-mining}}, we may extend it to Windows in the future. Besides, Java 8 should be installed before running the tool and DLV~\footnote{\url{http://www.dlvsystem.com/html/DLV_User_Manual.html}} is necessary for conducting experiment. In this chapter, the setting and command line interface of RUMIS are introduced, after that, features of the system are described in details.

\subsection{Setting}

\textbf{DLV.} This tool is used to extend a KG given a set of Horn or nonmonotonic rules at hand. RUMIS exploits DLV in its experiment function, i.e, a RUMIS' feature for evaluating result.
...
\textbf{Working Folder.} Working folder is a location for experiment where we have learning and approximated ideal KGs in SPO format (training.data.txt and ideal.data.txt, resp.), positive rule and sampled positive rule files (horn-rules.txt and selected.horn-rules.txt, resp.) in format~\ref{form2}. Besides, DLV binary file\footnote{\url{http://www.dlvsystem.com/files/dlv.x86-64-linux-elf-static.bin}} should be downloaded and renamed to dlv.bin in this folder. The selected.horn-rules.txt containing a subset of rules in horn-rules.txt, it is only necessary if we want to revise some rules in this file.
...
\begin{itemize}
\item -d: This flag is to enable DLV in order to extend KG.
\item -e=[execution function]: This requires a string as a function for execution, i.e., new, pos and neg, exp are corresponding to creating a new learning KG, positve and nonmonotonic rule mining and conducting experiment, resp.
\item -f=[working folder path]: This requires a string as an experiment folder path.
\item -h: This is the command line interface description.
\item -l=[KG file path]: This requires a string as a KG file path in SPO format.
\item -o=[predicate ratio]: This requires a percentage as a predicate ratio to create a new learning KG.
\item -p=[Horn rule file path]: This requires a string as an Horn rule file path in format~\ref{form2}.
\item -r=[ranking]: This requires an integer as a ranking type, i.e., 0, 1, 2 for Naive, PM, OPM ranking, resp.
\item -s: This flag is to enable sampling positive rules.
\item -t=[number of top Horn rules]: This requires an integer as a number of positive rules with top absolute support.
\end{itemize}

\subsection{Usage Description}

First of all, please download the repository\footnote{\url{https://github.com/htran010589/nonmonotonic-rule-mining}}, and then uncompress data/sample.imdb.txt.zip to get sample.imdb.txt file. In the next step, the repository folder should be located: \textit{\$ cd nonmonotonic-rule-mining}. Now we finish the preparation, and ready to describe the main features of RUMIS in details.

\textbf{Generating Learning KG.} Please generate the learning data in the format 1 with the following command: \textit{\$ java -jar rumis-1.0.jar -e=new -l=[path to KG file] -o=[ratio] 1>[training KG file path]}.

\textit{Example.} A learning KG of the IMDB sample dataset can be generated with predicate ratio being 80\%: \textit{\$ java -jar rumis-1.0.jar -e=new -l=data/sample.imdb.txt -o=0.8 1>training.sample.imdb.txt}. Then training.sample.imdb.txt file is the learning KG that we want to generate.
...
\textbf{Experiment.} First of all, create the working folder indicated above, and then run the following command for the experiment: \textit{java -XX:-UseGCOverheadLimit -Xmx[max memory]G -jar rumis-1.0.jar -e=exp -f=[working folder] -r=[ranking] -t=[top positive rules] -d -s 1>experiment.txt} where \textit{-XX, -Xmx, -t, -d, -s} are optional. -XX and -Xmx are used when we want to allocate more memory for RUMIS, and, if the option -t is not used, all the rules in \textit{horn-rules.txt} are revised. Besides, \textit{-s} option should only be added to the command if we just want to care about revisions of some selected rules in \textit{selected.horn-rules.txt} file.
