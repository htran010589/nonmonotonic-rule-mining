...
\begin{itemize}
\item \textit{Training data generation:} Given the ideal KG, the training KG can be chosen automatically where 20\% facts of each binary predicate are removed.
\item \textit{Horn rule mining:} With the training KG, a list of Horn rules can be learned with the introduced format in Algorithm~\ref{algo1}.
