...
the predictive power is improved~\cite{rumis}}
...
\includegraphics[width=0.5\textwidth]{big_pic}}
\end{picture}}
...
\smallskip
Consider the rule $r$: \textcolor{darkgreen}{\textit{livesIn(X,Z) $\leftarrow$ isMarriedTo(X,Y),livesIn(Y,Z)}}

\smallskip
\only<1>{
Its normal set $NS(r)$ is \textit{$\langle$Brad, Berlin$\rangle$}, \textit{$\langle$John, Chicago$\rangle$}, \textit{$\langle$Sue, Beijin$\rangle$}
...
\only<2>{
Its abnormal set $ABS(r)$ is \textit{$\langle$Alice, Berlin$\rangle$}, \textit{$\langle$Dave, Chicago$\rangle$}, \textit{$\langle$Lucy, Beijin$\rangle$}
