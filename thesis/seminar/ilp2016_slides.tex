...
\begin{frame}\frametitle{Motivation}
\centerline{\alt<3->{\bl{In this thesis:} revision rule mining under \alert{OWA}}{\bl{Complete} KGs with \bl{positive rule} mining \cite{amie}}}
 \bigskip
 \bigskip
 \bigskip
 \bigskip
...
\small{
${\alert{\cR^{\mi{aux}}_{\mi{NM}}}}{\alert{\cR_{\mi{NM}}}}=\left\{
            \renewcommand{\arraystretch}{1.1}
             \begin{array}{@{\,}l@{~~}l@{}}
               \mi{\gr{r1:\,livesIn(X,Z) \leftarrow isMarriedTo(X,Y),livesIn(Y,Z),}} \\ \mi{\alert{\naf\ Researcher(X)}}\\
              \mi{r1^{aux}:\,not\_livesIn(X,Z) \leftarrow isMarriedTo(X,Y),livesIn(Y,Z),} \\ \mi{Researcher(X)}\\
