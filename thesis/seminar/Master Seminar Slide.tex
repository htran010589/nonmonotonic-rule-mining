...
\begin{frame}\frametitle{Motivation}
\centerline{\alt<3->{\bl{In this thesis:} nonmonotonic rule mining under \alert{OWA}}{\bl{Complete} KGs with \bl{positive (Horn) rule} mining \cite{amie}}}
 \bigskip
 \bigskip
 \bigskip
 \bigskip
...
\begin{frame}{Outline}
\bl{ \textbf{Part I.}} Problem Statement
\bigskip
 \bigskip

\bl{\textbf{Part II.}} RUMIS System
\bigskip
\bigskip

\bl{\textbf{Part III.}} Experiments
\bigskip
 \bigskip

\bl{\textbf{Part IV.}} Summary
...
\only<2-3>{
\noindent \textbf{Find:} Revision of Horn rules with negation to improve quality
}
...
\only<2>{
\begin{picture}(0.5,0.5)\put(153,28){
\includegraphics[width=0.5\textwidth]{problem_2_new}}
\end{picture}
}

\only<3>{
\begin{picture}(0.5,0.5)\put(153,28){
\includegraphics[width=0.5\textwidth]{problem_3_new}}
\end{picture}
}
...
\begin{beamerboxesrounded}[upper=uppercolgreen,lower=lowercolgreen,shadow=true]{Keep the number of conflicts as small as possible}\bigskip
...
\item number of \bl{conflicting predictions} generated by the revision is as \textbf{small} as possible\smallskip

\item average \bl{conviction} $(\mi{\dfrac{1-support}{1-confidence}})$ is as \textbf{large} as possible~\footnotesize{\cite{rulemeasures}}
...
\smallskip
\bigskip
\centerline{\textcolor{darkgreen}{\textit{livesIn(X,Z), isMarriedTo(X,Y),livesIn(Y,Z)}}: 3 substitutions}
\centerline{\textcolor{darkgreen}{\textit{bornIn(X,Z), isMarriedTo(X,Y),bornIn(Y,Z)}}: 1 substitution}
...
\smallskip
\bigskip
\centerline{\textcolor{darkgreen}{\textit{livesIn(X,Z) $\leftarrow$ isMarriedTo(X,Y),livesIn(Y,Z)}}}
\centerline{\textcolor{darkgreen}{\textit{bornIn(X,Z) $\leftarrow$ isMarriedTo(X,Y),bornIn(Y,Z)}}}
...
\smallskip
\centerline{\textcolor{blue}{Normal set} for $\langle$X, Z$\rangle$: $\{$\textit{$\langle$Brad, Berlin$\rangle$}, \textit{$\langle$John, Chicago$\rangle$}, \textit{$\langle$Sue, Beijin$\rangle$}$\}$}
...
\smallskip
\centerline{\textcolor{red}{Abnormal set} for $\langle$X, Z$\rangle$: $\{$\textit{$\langle$Alice, Berlin$\rangle$}, \textit{$\langle$Dave, Chicago$\rangle$}, \textit{$\langle$Lucy, Ams$\rangle$}$\}$}
...
We propose a local optimum solution for the problem statement
\end{beamerboxesrounded}
\begin{itemize}
...
\smallskip
\begin{itemize}
\item Positive Horn rules ($\cR_H$)
\item Revisions ($\cR_N, \cR_{PM}, \cR_{OPM}$)
\item Blue area is nearly empty
\item Last column for green part
\item Sample 10 facts for each\\head relation
\end{itemize}

\begin{picture}(0.37,0.37)\put(155,05){
\includegraphics[height=0.37\textheight]{big_picture_exp_latest}}
\end{picture}
