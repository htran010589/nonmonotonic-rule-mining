...
Revised rules can be seen from generated file revised-rules.txt in the same folder with RUMIS jar file. Note that horn-rules.txt is generated by using above Horn rule mining function, or by another software. However, RUMIS currently only supports Horn rules in format 2.

The file revised-rules.txt list the revisions of Horn rules from horn-rules.txt with the ranking option being specified in the command. Two main sections are presented in revised-rules.txt, the first one describes exceptions ranked for each Horn rule and the second one lists final selected revisions. As regards the first section, Naive ranking is always presented on top of the file, followed by [PM | OPM] ranking if [PM | OPM] is selected, resp.

The first section contains many parts separated by a blank line, each of them describes rules with exceptions. The first line of each part is a positive rule and its measure values with the format: <rule> <Conv> <Conf>. Conv and Conf are abbreviations of conviction and confidence measures, resp. Please refer [1] for definition of conviction and confidence measures.

The rest of each part is top 10 negated atoms for each Horn rule which are sorted in the decreasing order of the positive-negative conviction (PosNegConv) of the corresponding revision achieved by inserting the negated atom to the rule [1]. The format for describing each exception with its measures is: <not exception> <PosNegConv> <Conv>.

The second section of the revised-rules.txt lists chosen revisions for all Horn rules. They are corresponding to the 1st rank exceptions of positive rules given the chosen ranking option in the first section.
